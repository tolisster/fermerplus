...
// Обрабатываем текст        
        ...
        {                   
			...
			$text = preg_replace_callback('#\[url=mailto://([^\] ]+?)\](.+?)\[/url\]#si', 'createLink3', $text);
        }    
        //Картинки. Тоже http:// и без пробелов. На вс. случай запретим GET параметры      
        if($images)    
            $text = preg_replace_callback('#\[img\]([^\] \?]+?)\[/img\]#si', 'createImg', $text); 
			
        //Картинки. Тоже http:// и без пробелов. На вс. случай запретим GET параметры      
       
	    if($images)    
            $text = preg_replace_callback('#\[zoom\]([^\] \?]+?)\[/zoom\]#si', 'createZoom', $text); 
			
		if($images)    
            $text = preg_replace_callback('#\[zoomgallery\]([^\] \?]+?)\[/zoomgallery\]#si', 'createZoomGallery', $text); 
...
*/ 
    ...
    {  
	    ...
	    function createLink3($match)
    {  
        $match[2] = str_replace("\n", "", $match[2]);
        return '<a href="mailto://'. htmlspecialchars($match[2]) 
             . '" target="_blanck" >'. htmlspecialchars($match[2]) .'</a>'; 
    }
...
*/
    ...
    {  
        $match[1] = str_replace("\n", "", $match[1]);
        return '<img src="'. htmlspecialchars($match[1]) .'" border="0" alt="lagmar Impex" />'; 
    }
	
	
	    function createZoom($match)
    {  
        $match[1] = str_replace("\n", "", $match[1]);
  return '<a id="example2" href="'. htmlspecialchars($match[1]) .'"><img src="'. htmlspecialchars($match[1]) .'" border="0" alt="lagmar Impex" /></a>'; 
    }
	
	
	
	 function createZoomGallery($match)
    {  
        $match[1] = str_replace("\n", "", $match[1]);
  return '<a rel="example_group" href="/uploads/big/'. htmlspecialchars($match[1]) .'" title="'. MAIN_TITLE .'"><img src="/uploads/'. htmlspecialchars($match[1]) .'" border="0" alt="lagmar Impex" /></a>'; 
    }
